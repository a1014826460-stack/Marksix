...
// 代次计数器：每次 switchLottery 自增，防止旧请求回调覆盖新数据
window.__ajaxGeneration = window.__ajaxGeneration || 0;
window.__LEGACY_SWITCH_SEQ__ = Number(window.__LEGACY_SWITCH_SEQ__ || 0);
...
$.ajaxPrefilter(function(options, originalOptions, jqXHR) {
    ...
    window.__moduleXHRs.push(jqXHR);

    // 拦截返回的数据
    jqXHR.done(function(data) {
        // 如果代次已变，说明用户切换了彩种，丢弃旧响应
        if (window.__ajaxGeneration !== requestGeneration || requestSeq !== Number(window.__LEGACY_SWITCH_SEQ__ || 0)) {
            return data;
        }
    ...
    jqXHR.always(function() {
        var idx = window.__moduleXHRs.indexOf(jqXHR);
