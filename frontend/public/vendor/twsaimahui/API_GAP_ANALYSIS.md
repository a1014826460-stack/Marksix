...
| 类别 | 数量 | 严重程度 | 状态 |
|---|---|---|---|
| 后端 Python 完全缺失的端点 | 0 | P0 | ✅ 已修复 |
| Next.js 兼容层未映射的 kaijiang 端点 | 0 | P0 | ✅ 全部 48 个端点已显式映射 |
| Next.js 兼容层路径名拼写不匹配 | 0 | P0 | ✅ 已修复 |
| 响应字段不完整 | 0 | P1 | ✅ ajax_interceptor 已全局保护 15+ 字段 |
| 特殊字段需求（兼容层无处理逻辑） | 0 | P1 | ✅ mapRowsWithFields 通用映射已覆盖 |
| `/api/legacy/module-rows` 自身限制 | 0 | P2 | ✅ 六不中专用表已创建 |
...
| 108 | getDxztt1 |
| 151 | getXmx1 / getXysxma（九肖一码） |
| 197 | getSanqiXiao4new（三期必中） |
| 244 | yyptj |
| 246 | qxbm |
| 331 | getPmxjcz |

### 9.2 2026-05-16 新确认的 modes_id 映射（从 SQLite DB）

| modes_id | 端点 | 中文名称 | 特殊字段 | 前端 JS 文件 |
|---|---|---|---|---|
| 5 | getTdsx1 | 天地生肖 | — | 043tiandi.js, 075tiandi.js |
| 10 | getFyld | 风雨雷电 | — | 051fyld.js |
| 15 | getDsxiao | 单双中特 | `xiao` | 004danshuang.js |
| 24 | getNnnx | 男女中特 | `nan`, `nv` | 020nn4x.js |
| 30 | getDsWei | 单双各4尾 | `dan`, `shuang` | 003ds4w.js |
| 39 | getCypt | 成语平特 | `title` | 066chengyupx.js |
| 41 | getShatou | 绝杀1头 | — | 018sha1tou.js |
| 54 | getPtWei | 平特1尾 | — | 022pt1w.js |
| 88 | getShama | 杀7码 | — | 056s7m.js |
| 123 | getWeima2 | 4尾8码 | — | 026siw8m.js |
| 132 | getHeds | 合单双 | — | 006heshuds.js |
| 141 | getYysx | 阴阳生肖 | — | 044yinyang.js |
| 143 | getYbzt | 1波中特 | — | 036ma12.js |
| 144 | getWwx | 文武生肖 | — | 046wenwu.js |
| 145 | getX2jiam8 | 2肖加8码 | `code` | 069lxbm.js |
| 147 | getYwx | 有无肖 | — | 045youwu.js |
| 149 | getBmzy | 笔墨纸砚 | — | 053wfsb.js |
| 152 | getZyx | 左右生肖 | — | 033zuoyou.js |
| 155 | getJmxc | 吉美凶丑 | — | 041meichou.js |
| 157 | getFsx | 肥瘦肖 | — | 034feishou.js |
| 158 | getDxd | 大小胆肖 | — | 037dandaxiao.js |
| 159 | getShaBds | 杀半单双 | — | 055sbands.js |
| 251 | getJyxiao2 | 家野两肖 | `xiao` | 061jy2x.js |
| 295 | getYzxj | 一字玄机 | `jiexi`, `xiao`, `zi` | 029yizixuanji.js |
| 336 | getCyptwei | 成语平特尾 | `title` | 068chengyupw.js |

**未找到 modes_id 的端点：无 — 全部 48 个端点已确认映射。**

| 端点 | modes_id | JS 文件 | 说明 |
|---|---|---|---|
| rd70i73lziizczak/0gmqnw/1 | **333** | 019liubuzhong.js | 六不中 — ✅ 已新建 `mode_payload_333` 表（含 `u6_code` 列），注册在 `database/schema/legacy.py:ensure_liubuzhong_table()` |
