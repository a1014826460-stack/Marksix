(function (window, document) {
  ...
  var predictionTitles = ["msks", "wsxx", "yxym", "wl4x", "dxzt", "jxzt", "jz5x", "5wzt", "jx24m", "dssx", "sdzt", "ybzt", "tdsx", "3tzt", "hsdx", "pt1xiao", "hsds", "qqsh"].map(function (id) {
    var root = document.getElementById(id);
    var node = root && root.querySelector(".tit, .list-title");
    return node ? { node: node, title: node.textContent.trim() } : null;
  }).filter(Boolean);
  ...
  function updatePredictionTitles(lotteryType) {
    var lottery = siteConfig.lotteries.filter(function (item) { return item.lotteryType === lotteryType; })[0];
    if (!lottery) return;
    predictionTitles.forEach(function (item) {
      var prefix = item.title.indexOf("精选") !== -1 ? lottery.titlePrefix : lottery.titleRegionPrefix;
      item.node.textContent = prefix + " " + item.title;
    });
  }
  ...
  function selectLottery(lotteryType) {
    ...
    updatePredictionTitles(lotteryType);
