...
def _get_effective_collect_url(
    ...
    def _resolve_backup(cfg_key: str, env_key: str) -> str:
        return str(_cfg(db_path, cfg_key, "") or os.environ.get(env_key, "")).strip()
    ...
    # 多个备用源按顺序用逗号合并（第一备用在前），由 result_crawler 分裂后逐个尝试。
    backups = ",".join(
        u for u in (
            _resolve_backup(backup_cfg_key, backup_env_key),
            _resolve_backup(backup2_cfg_key, backup2_env_key),
        ) if u
    )
    ...
    if (prefer_backup or fail_count >= threshold) and backups:
...
def _has_backup_collect_url(db_path: str | Path, lottery_type_id: int) -> bool:
    ...
    return bool(
        ...
        or str(_cfg(db_path, backup2_cfg_key, "") or os.environ.get(backup2_env_key, "")).strip()
    )
...
def _do_precise_draw_check(lottery_type_id: int, db_path: str) -> None:
    ...
    for attempt in range(4):  # 首次 + 3 次重试
        ...
        period, error = _fetch_current_draw_period(
            lottery_type_id, db_path, expected_period=expected_period,
        )
    ...
    # 全部重试失败，触发告警
    final_period, final_error = _fetch_current_draw_period(
        lottery_type_id, db_path, expected_period=expected_period,
    )
    ...
    def _do_precise_draw_fetch_and_open(self, lottery_type_id: int) -> dict[str, Any]:
        ...
        try:
            records = _fetch_current_draw_records(
                self.db_path,
                lottery_type_id,
                prefer_backup=matched_on_backup,
                expected_period=expected_period,
            )
    ...
    def _auto_crawl(self) -> None:
        ...
        for lt_row in lt_rows:
            ...
            try:
                ...
                cfg_prefix = _LT_CFG_PREFIX.get(lt_id, "")
                current_period = str(_cfg(self.db_path, f"{cfg_prefix}_current_period", ""))
                expected_period = _compute_next_period(current_period) if current_period else None
