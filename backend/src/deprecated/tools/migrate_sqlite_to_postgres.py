"""Legacy migration tool: copy an old Liuhecai SQLite database into PostgreSQL.

该脚本仅用于历史 SQLite 数据迁移到 PostgreSQL，不参与后台服务正式运行。
正式运行数据库已经统一为 PostgreSQL；如需迁移旧 SQLite 备份，请显式传入
`--source-sqlite` 和 `--target-dsn`。
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Legacy migration tool: migrate an explicit SQLite backup into PostgreSQL."
    )
    parser.add_argument(
        "--source-sqlite",
        required=True,
    ...
    parser.add_argument(
        "--target-dsn",
        default=str(os.environ.get("DATABASE_URL") or "").strip(),
...
if __name__ == "__main__":
    ...
    if not str(args.target_dsn or "").strip():
        raise ValueError("target_dsn 不能为空；请显式传入 --target-dsn 或设置 DATABASE_URL。")
