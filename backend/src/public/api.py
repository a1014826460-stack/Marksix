...
def _flat_zodiac_outcome(row: dict[str, Any]) -> str:
    """平特口径的真实命中目标：开奖 7 个号码对应的生肖集合。"""
    return "|".join(split_csv(row.get("res_sx")))


def _flat_tail_outcome(row: dict[str, Any]) -> str:
    """平特尾口径的真实命中目标：开奖 7 个号码对应的尾数集合。"""
    return "|".join(
        f"{int(code) % 10}尾" for code in split_csv(row.get("res_code")) if code.isdigit()
    )
...
def _check_correct_by_mechanism(
    ...
    if not content_labels:
        return None
    # 平特玩法：开奖 7 个号码的生肖/尾数集合与预测集合取交集。
    if flat_zodiac:
        return bool(config.hit_checker(_flat_zodiac_outcome(row), content_labels))
    if flat_tail:
        return bool(config.hit_checker(_flat_tail_outcome(row), content_labels))
