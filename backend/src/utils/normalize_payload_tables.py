...
def normalize_payload_tables(db_path: str | Path = DEFAULT_DB_TARGET) -> dict[int, str]:
    created_at = datetime.now(timezone.utc).isoformat()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Rebuild mode_payload_* tables from fetched_mode_records for PostgreSQL runtime."
    )
    parser.add_argument(
        "--db-path",
