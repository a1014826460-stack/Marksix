...
def _site_matches_twssz(site: dict[str, Any] | None) -> bool:
    if not site:
        return False
    if _normalize_domain(site.get("domain")) in {"www.twssz.com", "twssz.com"}:
        return True
    try:
        return int(site.get("web_id") or 0) == 9
    except (TypeError, ValueError):
        return False


def _site_matches_twbst528(site: dict[str, Any] | None) -> bool:
    if not site:
        return False
    if _normalize_domain(site.get("domain")) in {"www.twbst528.com", "twbst528.com"}:
        return True
    try:
        return int(site.get("web_id") or 0) == 10
    except (TypeError, ValueError):
        return False


def _site_matches_twjsz666(site: dict[str, Any] | None) -> bool:
    if not site:
        return False
    if _normalize_domain(site.get("domain")) in {"www.twjsz666.com", "twjsz666.com"}:
        return True
    try:
        return int(site.get("web_id") or 0) == 11
    except (TypeError, ValueError):
        return False
...
def _site_matches_twsyw(site: dict[str, Any] | None) -> bool:
    if not site:
        return False
    if _normalize_domain(site.get("domain")) in {"www.twsyw.com", "twsyw.com"}:
        return True
    try:
        return int(site.get("web_id") or 0) == 13
    except (TypeError, ValueError):
        return False
...
def get_required_mode_ids_for_site(site: dict[str, Any] | None) -> tuple[int, ...]:
    ...
    if _site_matches_twssz(site):
        return required_mode_ids_for_site_key("twssz")
    if _site_matches_twbst528(site):
    ...
    if _site_matches_twsyw(site):
        return required_mode_ids_for_site_key("twsyw")
...
def get_known_unavailable_mode_ids_for_site(site: dict[str, Any] | None) -> tuple[int, ...]:
    ...
    if _site_matches_twssz(site) or _site_matches_twbst528(site) or _site_matches_twjsz666(site) or _site_matches_twwanli(site) or _site_matches_twsyw(site):
        return ()
