...
from domains.prediction.site_module_blueprints import TWCF888_REQUIRED_MODE_IDS  # noqa: E402
...
def _parse_site_ids(value: str) -> list[int]:
    ...
    return sorted(set(site_ids)) or [5, 6, 7, 8]
...
def _run_audit(conn, site_ids: list[int]) -> dict:
    ...
    return {
        ...
        "twcf888_document_matches_blueprint": (
            parse_twcf888_document_mode_ids(twcf888_document) == set(TWCF888_REQUIRED_MODE_IDS)
        ),
...
def main() -> None:
    ...
    parser.add_argument("--site-ids", default="5,6,7,8", help="Comma-separated managed_sites IDs")
    ...
    # Run the idempotent schema migration so legacy `default` profiles are
    # promoted to their dedicated site blueprints before authorization changes.
