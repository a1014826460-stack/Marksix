...
无路径的兼容接口 `/api/prediction-modules` 必须使用一个显式注册身份：`site_key` 或 `site_id`。两者同时提供时必须映射到同一注册站点；冲突返回现有错误 envelope，不能再回退到默认站点或跨站读取。
...
- 2026-07-19：正式 PostgreSQL 对站点 5-8 执行显式同步。仅站点 5 有变化：`5`、`44`、`69`、`108`、`151`、`251` 从启用改为停用；未删除任一模块行或预测历史。
- 同步后，站点 5-8 的启用 `mode_id` 集合均与各自专属蓝图一致。
- `twcaibawang` vendor 复合块继续保留原有对象字段；其来源模式未启用时返回空 `history`，不再绕过授权读取数据。
- 验证：目标授权/蓝图/legacy 契约集 `43 passed`；完整后端回归 `412 passed, 10 skipped`。
